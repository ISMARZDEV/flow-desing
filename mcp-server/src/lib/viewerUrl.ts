/**
 * Build a shareable URL that opens the AISpaceFlow viewer with the given DSL
 ...
 *
 * Override the base origin with AISPACEFLOW_APP_URL (e.g. http://localhost:5173
 * for local dev). Defaults to https://app.aispaceflow.com. The app currently
 * uses hash routing, so the viewer path must live after `#/`.
 */
import { deflate } from 'pako';

const DEFAULT_APP_URL = 'https://app.aispaceflow.com';
const PAKO_PREFIX = '~';

function getAppBaseUrl(): string {
  const fromEnv = process.env.AISPACEFLOW_APP_URL?.trim();
...
/** URL that opens the DSL in AISpaceFlow's read-only viewer. */
