...
describe('aiSettings', () => {
  afterEach(() => {
    localStorage.removeItem('aispaceflow-ai-settings');
    localStorage.removeItem('aispaceflow-ai-settings-secret');
    sessionStorage.removeItem('aispaceflow-ai-settings');
    sessionStorage.removeItem('aispaceflow-ai-settings-secret');
  });
  ...
  it('persists and reloads sanitized AI settings from local storage by default', () => {
    ...
    expect(localStorage.getItem('aispaceflow-ai-settings')).not.toContain('secret');
    expect(localStorage.getItem('aispaceflow-ai-settings-secret')).toBeTruthy();
  ...
  it('persists session-only AI settings in sessionStorage and clears local copy', () => {
    localStorage.setItem('aispaceflow-ai-settings', JSON.stringify({
      provider: 'gemini',
      storageMode: 'local',
      apiKey: 'local-secret',
    }));
    ...
    expect(localStorage.getItem('aispaceflow-ai-settings')).toBeNull();
    expect(JSON.parse(sessionStorage.getItem('aispaceflow-ai-settings') ?? '{}')).toMatchObject({
      provider: 'openai',
      storageMode: 'session',
      model: 'gpt-4o',
    });
    expect(sessionStorage.getItem('aispaceflow-ai-settings')).not.toContain('session-secret');
    expect(sessionStorage.getItem('aispaceflow-ai-settings-secret')).toBeTruthy();
  ...
  it('reloads legacy plaintext api keys for compatibility while rewriting newer writes to secret storage', () => {
    ...
    expect(localStorage.getItem('aispaceflow-ai-settings')).not.toContain('legacy-secret');
    expect(localStorage.getItem('aispaceflow-ai-settings-secret')).toBeTruthy();
  });
  ...
  it('can clear persisted AI settings from both storage buckets', () => {
    ...
    sessionStorage.setItem('aispaceflow-ai-settings', JSON.stringify({
      provider: 'openai',
      storageMode: 'session',
      apiKey: 'session-secret',
    }));
    ...
    expect(localStorage.getItem('aispaceflow-ai-settings')).toBeNull();
    expect(sessionStorage.getItem('aispaceflow-ai-settings')).toBeNull();
    expect(loadPersistedAISettings()).toEqual(DEFAULT_AI_SETTINGS);
  });
});
